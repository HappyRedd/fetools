var sharedata = {
    ...
    TimelineLink: window.location.href.split('#').shift().replace(/game.aspx/, 'index.aspx'),
    TimelineImgUrl: window.location.href.split('?').shift().split('#').shift().replace(/game.aspx?/, '') + 'images/share.jpg',

    AppMessageTitle: "眼明手快大闯关，朗沐上市3周年庆典邀请函速get√",
    AppMessageDesc: "看谁能火眼金，闯三关啦！不服来战！",
    AppMessageLink: window.location.href.split('#').shift().replace(/game.aspx/, 'index.aspx'),
    AppMessageImgUrl: window.location.href.split('?').shift().split('#').shift().replace(/game.aspx?/, '') + 'images/share.jpg'
};
...
var sceneIDArray = ['scene02', 'scene03', 'scene04', 'scene05', 'scene06', 'scene07', 'scene08', 'scene09', 'scene10'];
var sceneIDActions = [p_02_action, p_03_action, p_04_action, p_05_action, p_06_action, p_07_action, p_08_action, p_09_action, p_10_action];
